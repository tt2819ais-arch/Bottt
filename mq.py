...
def calculate_expression(expression: str) -> Optional[float]:
    ...
    try:
        ...
        # Заменяем деление на целочисленное если нужно
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Текстовое сообщение от {username}: {message_text}")
    
    # 1. Сначала проверяем калькулятор
...
def main():
    ...
    logger.info("Доступные команды: /start, /help, /drop, /ask, /rub, /stats")
